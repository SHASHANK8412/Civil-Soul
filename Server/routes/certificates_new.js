...
router.patch('/:id', [
  ...
  try {
    const { status, blockchainHash, certificateId, performanceScore, error } = req.body;
    ...
    if (blockchainHash) certificate.blockchainHash = blockchainHash;
    if (certificateId) certificate.certificateId = certificateId;
    if (performanceScore) certificate.performanceScore = performanceScore;
    if (error) certificate.error = error;
...
// @route   GET /api/certificates/template/:type
...
router.get('/template/:type', async (req, res) => {
  try {
    ...
    // Return template based on type
    const templates = {
      volunteering: {
        name: 'Volunteer Certificate Template',
        fields: ['volunteerName', 'organizationName', 'activityType', 'hoursCompleted', 'performanceScore'],
        design: 'modern',
        colors: ['#4F46E5', '#059669']
      },
      internship: {
        name: 'Internship Certificate Template',
        fields: ['volunteerName', 'organizationName', 'activityType', 'hoursCompleted', 'skills'],
        design: 'professional',
        colors: ['#7C3AED', '#DC2626']
      },
      completion: {
        name: 'Completion Certificate Template',
        fields: ['volunteerName', 'organizationName', 'programName', 'completionDate'],
        design: 'classic',
        colors: ['#059669', '#F59E0B']
      }
    };

    const template = templates[type] || templates.volunteering;
